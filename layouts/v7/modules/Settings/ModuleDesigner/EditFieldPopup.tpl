...
<h5 class="modal-title">{$a_field.UITypeNum} - {vtranslate($a_field.UITypeName, $QUALIFIED_MODULE)}</h5>
...
<tr class="related_module">
	...
	<td colspan="2">
		...
		<a href="#" onclick="md_deleteRelatedModule(this); return false;" {if $key == 0}style="display:none;"{/if} class="delete-related-module"><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/delete.png" alt="{vtranslate('LBL_DELETE_RELATED_MODULE', $QUALIFIED_MODULE)}" /></a>
	</td>
</tr>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/mandatory.png" alt="{vtranslate('LBL_MANDATORY_ALT', $QUALIFIED_MODULE)}" /> {vtranslate('LBL_MANDATORY', $QUALIFIED_MODULE)}</td>
	<td colspan="2"><input type="checkbox" name="mandatory" value="1" {if $a_field.mandatory}checked="checked"{/if} /></td>
</tr>
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/entityidentifier.png" alt="{vtranslate("LBL_ENTITY_IDENTIFIER_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_ENTITY_IDENTIFIER", $QUALIFIED_MODULE)}</td>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/filterall.png" alt="{vtranslate("LBL_FILTER_ALL_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_IN_FILTER_ALL", $QUALIFIED_MODULE)}</td>
	<td colspan="2"><input type="checkbox" name="in_filter_all" value="1" {if $a_field.inFilterAll}checked="checked"{/if} /></td>
</tr>
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/popup.png" alt="{vtranslate("LBL_IN_POPUP_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_IN_POPUP", $QUALIFIED_MODULE)}</td>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/related.png" alt="{vtranslate("LBL_RELATED_LIST_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_IN_RELATED_LIST", $QUALIFIED_MODULE)}</td>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/create.png" alt="{vtranslate("LBL_QUICK_CREATE_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_QUICK_CREATE", $QUALIFIED_MODULE)}</td>
	<td colspan="2"><input type="checkbox" name="quick_create" value="1" {if $a_field.quickCreate}checked="checked"{/if} /></td>
</tr>
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/massedit.png" alt="{vtranslate("LBL_MASS_EDIT_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_MASS_EDITABLE", $QUALIFIED_MODULE)}</td>
	<td colspan="2"><input type="checkbox" name="mass_editable" value="1" {if $a_field.massEditable}checked="checked"{/if} /></td>
</tr>
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/readonly.png" alt="{vtranslate("LBL_READ_ONLY_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_READ_ONLY", $QUALIFIED_MODULE)}</td>
...
{ldelim}
	...
	o_data.UITypeName					= '{($a_field.UITypeName)|escape:"javascript"}';
	o_data.UITypeDBType					= o_data.UITypeNum == 7 && $("input[name='numeric_type']:checked").val() != 'I' ? 'DECIMAL(25,3)' : $("input[name='column_type']").val();
	o_data.UITypeDataType				= o_data.UITypeNum == 7 ? $("input[name='numeric_type']:checked").val() : '{($a_field.UITypeDataType)|escape:"javascript"}';
	o_data.twoColumns					= {if $a_field.twoColumns}true{else}false{/if};
	o_data.fieldName					= $("input[name='field_name']").val();
	o_data.oldFieldName					= '{($a_field.fieldName)|escape:"javascript"}';
...
{/foreach}
	...
	if(o_data.label == '' || o_data.label == 'LBL_')
		field = '{(vtranslate("LBL_FIELD_LABEL", $QUALIFIED_MODULE))|escape:"javascript"}';
{foreach item=language from=$a_languages}
	else if(o_data.label_{$language} == '')
		field = '{(vtranslate("LBL_FIELD_LABEL_TRANSLATION", $QUALIFIED_MODULE)|cat:' '|cat:$language)|escape:"javascript"}';
{/foreach}
else if(o_data.isEntityIdentifier && o_data.entityIdentifierFieldName == '')
	field = '{(vtranslate('LBL_FIELD_ENTITY_IDENTIFIER', $QUALIFIED_MODULE))|escape:"javascript"}';
{foreach item=language from=$a_languages}
	else if(o_data.helpInfoLabel != '' && o_data.helpInfoLabel_{$language}  == '')
		field = '{(vtranslate('LBL_FIELD_HELP_INFO_LABEL_TRANSLATION', $QUALIFIED_MODULE))|escape:"javascript"} {$language}';
{/foreach}
	else if((o_data.UITypeNum == 15 || o_data.UITypeNum == 16 || o_data.UITypeNum == 33) && (o_data.pickListValues == ''))
                field = '{(vtranslate('LBL_OPTIONS', $QUALIFIED_MODULE))|escape:"javascript"}';
    else
		valid = true;
