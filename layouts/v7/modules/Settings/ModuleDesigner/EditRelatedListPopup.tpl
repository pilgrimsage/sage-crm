...
<h5 class="modal-title">{vtranslate('LBL_RELATED_LIST_LINK_TO_MODULE', $QUALIFIED_MODULE)} - {vtranslate($a_relatedList.relatedModule, $QUALIFIED_MODULE)}</h5>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/presence.png" alt="{vtranslate("LBL_RELATED_LIST_PRESENCE_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_RELATED_LIST_PRESENCE", $QUALIFIED_MODULE)}</td>
...
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/add.png" alt="{vtranslate("LBL_RELATED_LIST_ACTION_ADD_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_RELATED_LIST_ACTION_ADD", $QUALIFIED_MODULE)}</td>
	<td><input type="checkbox" name="action_add" value="1" {if $a_relatedList.actionAdd}checked="checked"{/if} /></td>
</tr>
<tr>
	<td><img src="layouts/v7/modules/Settings/{$MODULE}/resources/images/select.png" alt="{vtranslate("LBL_RELATED_LIST_ACTION_SELECT_ALT", $QUALIFIED_MODULE)}" /> {vtranslate("LBL_RELATED_LIST_ACTION_SELECT", $QUALIFIED_MODULE)}</td>
...
{/foreach}
	...
	if(o_data.label == '' || o_data.label == 'LBL_')
		field = '{(vtranslate("LBL_RELATED_LIST_LABEL", $QUALIFIED_MODULE))|escape:"javascript"}';
{foreach item=language from=$a_languages}
	else if(o_data.label_{$language} == '')
		field = '{(vtranslate("LBL_RELATED_LIST_LABEL_TRANSLATION", $QUALIFIED_MODULE)|cat:' '|cat:$language)|escape:"javascript"}';
{/foreach}
	else if(o_data.functionName == '')
		field = '{(vtranslate("LBL_RELATED_LIST_NAME", $QUALIFIED_MODULE))|escape:"javascript"}';
	else
		valid = true;
	...
	{ldelim}
		md_addRelatedList(o_data, false);
		md_closePopup();
	{rdelim}
{rdelim}
