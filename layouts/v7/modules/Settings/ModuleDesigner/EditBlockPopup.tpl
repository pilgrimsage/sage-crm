...
<h5 class="modal-title">{vtranslate("LBL_BLOCK", $QUALIFIED_MODULE)}</h5>
...
{/foreach}
	...
	if(o_data.label == '')
		field = '{(vtranslate("LBL_BLOCK_LABEL", $QUALIFIED_MODULE))|escape:"javascript"}';
{foreach item=language from=$a_languages}
	else if(o_data.label_{$language} == '')
		field = '{(vtranslate("LBL_BLOCK_LABEL_TRANSLATION", $QUALIFIED_MODULE)|cat:' '|cat:$language)|escape:"javascript"}';
{/foreach}
	else
		valid = true;
