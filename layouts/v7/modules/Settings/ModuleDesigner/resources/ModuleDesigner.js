	...
	function()
	{
		...
		(
			function(event)
			{
				...
				{
					app.helper.showAlertBox({message: app.vtranslate('LBL_SELECT_TEMPLATE', MD_QUALIFIED_MODULE_NAME)});
				}
				else if($(event.target).attr("id") != "md-tab-general" && jmd_container.find("select[name='module_manifest_template']").val() == '')
				{
					app.helper.showAlertBox({message: app.vtranslate('LBL_SELECT_MANIFEST', MD_QUALIFIED_MODULE_NAME)});
				}
				else if($(event.target).attr("id") != "md-tab-general" && jmd_container.find("input[name='module_name']").val() == '')
				{
					app.helper.showAlertBox({message: app.vtranslate('LBL_DEFINE_MODULE_NAME', MD_QUALIFIED_MODULE_NAME)});
				}
				else
				{
		...
		jmd_container.find("#md_related_list_other_module").click
		(
			function()
			{
				Swal.fire({
					title: app.vtranslate('LBL_RELATED_LIST_OTHER_MODULE', MD_QUALIFIED_MODULE_NAME),
					input: 'text',
					inputLabel: app.vtranslate('LBL_RELATED_LIST_OTHER_MODULE_NAME', MD_QUALIFIED_MODULE_NAME),
					showCancelButton: true
				}).then(function(result) {
					if(result.isConfirmed && result.value != '')
					{
						md_openPopup("index.php?module="+MD_MODULE_NAME+"&view=EditRelatedList&relmodule="+result.value);
					}
				});
     		 }
     	);
...
function md_dropToTrash(ui)
{
	...
	else if(className.search("md-filter") > -1)
	{
		...
		{
			app.helper.showAlertBox({message: app.vtranslate('LBL_CANNOT_DELETE_FILTER_ALL', MD_QUALIFIED_MODULE_NAME)});
			return;
		}
		...
		for(var i=0; i<a_array.length && !found; i++)
		{
			if(a_array[i].id == id)
			{
				...
				else if(className.search("md-field") > -1)
				{
					...
					if(editable !== 'true')
					{
						app.helper.showAlertBox({message: app.vtranslate('LBL_CANNOT_EDIT_REMOVE_FIELD', MD_QUALIFIED_MODULE_NAME)});
						error = true;
...
{        
	...
	$("#"+block_id+" li").each(function() {
		...
		if(editable !== 'true')
		{
			app.helper.showAlertBox({message: app.vtranslate('LBL_CANNOT_BLOCK_REMOVE_FIELD', MD_QUALIFIED_MODULE_NAME)});
			found = false;
...
function md_openPopup(url)
{
	...
	$.ajax({
		...
		error: function() {
			app.helper.showAlertBox({message: app.vtranslate('LBL_ERROR_TRY_AGAIN', MD_QUALIFIED_MODULE_NAME)});
		}
	});
}
	...
	(
		{
			...
			{	
				if(!response.success)
				{
					app.helper.showAlertBox({message: response.error.message});
				}
				else
				{
					var html = '';
...
function md_addField(o_field, isImporting)
{
	...
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.mandatory ? 'mandatory' : 'mandatory-bw')+'.png" class="'+(o_field.mandatory ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.isEntityIdentifier ? 'entityidentifier' : 'entityidentifier-bw')+'.png" class="'+(o_field.isEntityIdentifier ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.inFilterAll ? 'filterall' : 'filterall-bw')+'.png" class="'+(o_field.inFilterAll ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.inPopup ? 'popup' : 'popup-bw')+'.png" class="'+(o_field.inPopup ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.inRelatedList ? 'related' : 'related-bw')+'.png" class="'+(o_field.inRelatedList ? 'md-icon-active' : 'md-icon-inactive')+'" /><br /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.quickCreate ? 'create' : 'create-bw')+'.png" class="'+(o_field.quickCreate ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.massEditable ? 'massedit' : 'massedit-bw')+'.png" class="'+(o_field.massEditable ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.defaultValue != '' ? 'defaultvalue' : 'defaultvalue-bw')+'.png" class="'+(o_field.defaultValue != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.helpInfoLabel != '' ? 'helpinfo' : 'helpinfo-bw')+'.png" class="'+(o_field.helpInfoLabel != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_field.readOnly ? 'readonly' : 'readonly-bw')+'.png" class="'+(o_field.readOnly != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '</div>';
	row += '</li>';
	...
	(
		function()
		{
			...
			{
				app.helper.showAlertBox({message: app.vtranslate('LBL_NOT_ALLOWED_TO_EDIT_THE_FIELD', MD_MODULE_NAME)});
			}
		}
	);
...
function md_addCustomLink(o_customLink, isImporting)
{
	...
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_customLink.icon != '' ? 'icon' : 'icon-bw')+'.png" class="'+(o_customLink.icon != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_customLink.handlerPath != '' ? 'handler-path' : 'handler-path-bw')+'.png" class="'+(o_customLink.handlerPath != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_customLink.handlerClass != '' ? 'handler-class' : 'handler-class-bw')+'.png" class="'+(o_customLink.handlerClass != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_customLink.handler != '' ? 'handler' : 'handler-bw')+'.png" class="'+(o_customLink.handler != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '</div>';
...
function md_addRelatedList(o_relatedList, isImporting)
{
	...
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_relatedList.presence == 1 ? 'presence' : 'presence-bw')+'.png" class="'+(o_relatedList.presence == 1 ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_relatedList.actionAdd ? 'add' : 'add-bw')+'.png" class="'+(o_relatedList.actionAdd ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_relatedList.actionSelect ? 'select' : 'select-bw')+'.png" class="'+(o_relatedList.actionSelect != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '</div>';
...
function md_addEvent(o_event, isImporting)
{
	...
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_event.isActive ? 'active' : 'active-bw')+'.png" class="'+(o_event.isActive ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_event.cond != '' ? 'cond' : 'cond-bw')+'.png" class="'+(o_event.cond != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '<img src="layouts/v7/modules/Settings/'+MD_MODULE_NAME+'/resources/images/'+(o_event.dependentOn != '' ? 'dependent' : 'dependent-bw')+'.png" class="'+(o_event.dependentOn != '' ? 'md-icon-active' : 'md-icon-inactive')+'" /> ';
	row += '</div>';
...
function md_makePackage(installModule)
{
    ...
    if(md_moduleManifestTemplate === 'module.xml.php' && md_moduleDirectoryTemplate === '6.0.0')
    {
        ...
        if(errorEntity === true)
        {
            app.helper.showAlertBox({message: app.vtranslate('LBL_MANDATORY_ENTITYFIELD', MD_QUALIFIED_MODULE_NAME)});
        ...
        if(error_filters)
        {
            app.helper.showAlertBox({message: app.vtranslate('LBL_EMPTY_FIELD_FILTER', MD_QUALIFIED_MODULE_NAME)});
	...
	(
		{
			...
			success: function(data) {				
				if(data.success)
				{
					...
					if (zip_url) {
						$.ajax({
							...
							type:'HEAD',
							error: function() {
								app.helper.showAlertBox({message: app.vtranslate('LBL_FILE', MD_QUALIFIED_MODULE_NAME)+' '+zip_url+' '+app.vtranslate('LBL_DOES_NOT_EXIST', MD_QUALIFIED_MODULE_NAME)});
							},
							success: function() {
								if(installModule)
								{								
				...
				{
					console.log(data);
					app.helper.showAlertBox({message: data.error.message});
				}
			},
			error: function(data) {
				console.log(data);
	...
	(
		{
			...
			success: function(data) {				
				if(data.success)
				{
					app.helper.showAlertBox({message: data.result.message});
				}
				else
				{
					app.helper.showAlertBox({message: data.error.message});
